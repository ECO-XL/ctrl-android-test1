    ...
    public void doParse(Context context, String baseId, String data, boolean connectedStateChanged, boolean connected) {
        ...
        if (data == null) {
            if (connectedStateChanged) {
                if (connected)
                    showNotification(context, baseId, b.getTitle() + " is now Online", "Base is now Online! Click to interact with it.");
                else
                    showNotification(context, baseId, b.getTitle() + " is now Offline", "Base went Offline.");
            }
        }
        // Nope, this is some new data arrived
        else {
            // For example, temperature exceeded a pre-set value
            showNotification(context, baseId, b.getTitle() + " - temperature alert", "Temperature too high! Click to investigate.");
        }
    }
